...
def build_state(manager: TaxiManager) -> list[float]:
    """
    Fixed-length numeric state.
    This is what BC / offline RL can use later.

    Format:
    [
        taxi_x, taxi_y, carrying_flag,
        for each passenger:
            picked_up, completed,
            pickup_x, pickup_y,
            dropoff_x, dropoff_y,
...
def choose_step_toward(current: tuple[int, int], target: tuple[int, int]) -> tuple[int, int]:
    ...
    If both x and y differ, randomly pick one improving move.
    """
    cx, cy = current
    tx, ty = target
    ...
    if not candidates:
        # already at target; choose a random valid move shape
...
def choose_best_value_target(manager: TaxiManager) -> tuple[tuple[int, int], dict[str, Any]]:
    ...
    # If already carrying a passenger, go to their destination
    ...
    # Otherwise choose the best waiting passenger
    candidates = []
    ...
    if not candidates:
        # fallback if everything is done or weird edge case
...
def compute_reward(moved: bool, dropped_off: bool, payout: float) -> float:
    if not moved:
...
def generate_logs() -> None:
    ...
    with RAW_LOG_FILE.open("w", encoding="utf-8") as f:
        for event in raw_events:
            f.write(json.dumps(event) + "\n")

    with TRANSITIONS_FILE.open("wb") as f:
